/*
Small Large Split Merge sort is a variation of standard merge sort, 
where the 'n' (if n>2) elements in the sequence are split into two parts with 2 elements in the first part and (n-2) elements in the second part.
If n==2, then split them into two equal sorted sequences.
Given the partial code of standard merge sort in the code editor, modify the code such that Small Large Split Merge Sort is implemented. 
There is no change in the merge function, here we merge function similar to the standard merge sort.
*/
...
void mergesort(vector<int> &elements, int left, int right)
{
	...
	mergesort(elements,left,partition);
	// call mergesort for right subarray
	mergesort(elements,partition+1,right);
	// merge the sorted left and right subarray
	merge(elements,left,partition,right);
}
